...
import { ISalePartyItemST } from '../../AllReportsInterfaces/ISalePartyItemST';
...
@Component({
  selector: 'app-salepartyitemst',
  templateUrl: './salepartyitemst.component.html',
  styleUrls: ['./salepartyitemst.component.scss'],
})
export class SalepartyitemstComponent implements OnInit {
  saleRepo = new Array <ISalePartyItemST>();
  saleItemArr = new Array<ISalePartyItemST>();
  filterTerm: string = '';
  qtyTotal: number = 0;
  billTotal: number = 0;
  saleArray = [];
  ...
  getReport() {
    this.service
      .getPISaleWithSTReport(this.fDate, this.tDate)
      .subscribe((res) => {
        if (res) {
          this.isData = true;
          this.saleRepo = res;
