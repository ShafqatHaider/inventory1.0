...
import { IPurchasePartyItem } from '../../AllReportsInterfaces/IPurchasePartyItem';
import { Location } from '@angular/common';

@Component({
  selector: 'app-salepartyitem',
  templateUrl: './purchasepartyitem.component.html',
  styleUrls: ['./purchasepartyitem.component.scss']
})
export class PurchasepartyitemComponent implements OnInit {
  PurchaseRepo = new Array<IPurchasePartyItem>();
  PurchaseArr = new Array<IPurchasePartyItem>();
  filterTerm: string = "";
  qtyTotal: number = 0;
  billTotal: number = 0;
  purchaseArray = [];
  ...
  getReport() {
    this.service.getPIPurchaseReport(this.fDate, this.tDate).subscribe(res => {
      if (res) {
        this.isData = true;
