...
import { IPurchasePartyItemST } from '../../AllReportsInterfaces/IPurchasePartyItemST';

@Component({
  selector: 'app-purchasepartyitemst',
  templateUrl: './purchasepartyitemst.component.html',
  styleUrls: ['./purchasepartyitemst.component.scss']
})
export class PurchasepartyitemstComponent implements OnInit {

  PurchaseRepo = new Array<IPurchasePartyItemST>();
  PurchaseArr = new Array<IPurchasePartyItemST>();
  filterTerm: string = "";
  qtyTotal: number = 0;
  billTotal: number = 0;
  purchaseArray = [];
...
totalReturnAmt=0
  ...
  getReport() {
    this.service.getPIPurchaseSTReport(this.fDate, this.tDate).subscribe(res => {
      if (res) {
        this.isData = true;

        this.PurchaseRepo = res;
        this.purchaseArray = res;
        this.PurchaseArr = res;
        // 

        console.log(this.PurchaseRepo);
  ...
  createReportPdf() {
    let content = `<html>
    <head>
      <title>Party & Item Wise Purchase Report</title>
      <style>
    ...
    <h2>Party & Item Wise Purchase Report</h2>
    </div>
