...
import { INewSale, INewSaleDatail } from '../../shared/interface/INewSale';
import { environment } from 'src/environments/environment';
import { ISaleInvoice } from 'src/app/shared/interface/ISaleInvoice';
 ...
 this.SSub.amount=this.SSub.qty*(this.SSub.rate-(((this.SSub.rate*this.SSub.disPer)/100)+(this.SSub.disRS)))
  ...
  Save() {
    ...
    if (this.SMain.sAccID && this.SMain.salesDetail.length > 0) {
      this._IS.saveSale(this.SMain).subscribe(res=>{alert(res), err=> alert(err), this.SMain=new INewSale(), this.route.navigate(['/inventory/sale-index'])});
